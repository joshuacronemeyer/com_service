...
class PostmarkGatewayTest < Minitest::Test
  ...
  def test_postmark_400_raises
    stub_request(:post, PostmarkGateway::MAIL_URL).to_return(status: [400, 'That email address bounced.'])
    email = Email.new(to: 'george@example.com', to_name: 'George', from: 'martha@example.com', from_name: 'Martha', subject: "Let's go", body: 'I want to go.')
    error = assert_raises RuntimeError do
      PostmarkGateway.new.send_email(email: email)
    end
    assert_equal 'Postmark responded with: code-400 : message-That email address bounced. : body-', error.message
  end
